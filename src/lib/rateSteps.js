...
export function effectiveRateSteps(result) {
  ...
  const rows = [
    heading('step1', 'Step 1: income from everything except this account'),
    money('ssBenefit', 'Social Security benefit', ss.annualBenefit),
    money('otherPretax', 'Other Pre-tax accounts (4% withdrawal)', o.pretaxGross),
    ...(o.roth > 0 ? [money('otherRoth', 'Other Roth accounts (4%, tax-free)', o.roth)] : []),
    ...(hasGains ? [money('otherTaxable', 'Other taxable accounts (4% withdrawal)', o.taxableGross)] : []),
    ...
    money('afterTaxOther', 'After-tax income from other sources', g.afterTaxFromOtherSources, 'total'),

    heading('step2', 'Step 2: what this account has to supply'),
    money('need', 'Retirement income number', retirementNeed.target),
    money(
      'stillNeeded',
      'Still needed from this account, after tax',
  ...
  if (withdrawalNeeded) {
    ...
    rows.push(
      money('grossWithdrawal', 'Withdrawal needed from this account', 0, 'total'),
      heading('step3', 'Step 3: what a withdrawal from it would cost, if you took one'),
      money('probeSize', "This account's own natural withdrawal (4% of its projected value)", g.probeSize),
...
export function rateDriverRows(result) {
  ...
  return [
    ...
    money('driverIncomeFirst', 'Ordinary income taxed ahead of it (other Pre-tax + taxable Social Security)', d.ordinaryIncomeBefore),
